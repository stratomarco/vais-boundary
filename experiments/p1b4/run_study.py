"""Run the P1b-4 study arms in pre-registered order. Resumable; see PREREGISTRATION.md.

Usage, from the repository root, with LM Studio's server running:

    .venv\\Scripts\\python.exe experiments\\p1b4\\run_study.py
    .venv\\Scripts\\python.exe experiments\\p1b4\\run_study.py --dry-run
    .venv\\Scripts\\python.exe experiments\\p1b4\\run_study.py --arm qwen3.5-9b-on
...
def lms(*args: str, capture: bool = True) -> str:
    result = subprocess.run([lms_path(), *args], capture_output=capture, text=True)
...
def git_commit() -> str:
    ...
    dirty = subprocess.run(["git", "status", "--porcelain", "--", "src", "experiments/p1b4"],
