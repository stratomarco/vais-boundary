...
- **Add a stateful monitor.** A `SessionLedger` passed to the monitor records what it allowed for one session. With it, policy v5's `max_calls` is enforced in flight and a contract-held approval is single-use; a declared limit without a ledger is denied rather than ignored. The `monitor_mediated` invariant reads the same ledger to report effects no recorded `ALLOW` accounts for. Restructuring the approval path found and fixed FIND-056: a tool needing both an exact and a threshold approval consumed the store grant twice and could never be allowed (DEC-044 to DEC-046, LIM-055).
- **Let authority expire, be withdrawn and be handed on in part.** Contracts can carry `not_before`/`not_after`, grants a TTL, and a `RevocationList` withdraws a session or a capability, all checked before anything else. The approval store's file is locked and reloaded around every operation, so processes on one machine consume a grant exactly once. `TaskContract.delegate` derives a sub-agent contract that can only narrow (DEC-047 to DEC-050, LIM-056 to LIM-059).
- **Add the gateway.** `vais gateway` serves MCP over streamable HTTP and is the only holder of the upstream credentials. It takes contracts from operator files by token digest, labels every argument itself, lets only an operator grant approvals (`vais gateway-approve`), withholds decision reasons from the agent by default and keeps one audit trail. Complete mediation holds under the deployment conditions in `docs/gateway.md` (DEC-051 to DEC-054, LIM-060 to LIM-063).
- **Add action origin.** Actions and effects can carry the join of what was visible when the action was planned; policy v6's `untrusted_origin` can escalate or deny a tool whose action has an untrusted origin, and the `trusted_origin` invariant checks it in VERIFY. A pre-registered replay of RC7 found it untrusted for every model action in clean and attacked runs alike, so it is a per-tool human gate, not a detector, and not a default (FIND-057, DEC-055, DEC-056, LIM-064).
- **Add effect confidence.** Effects are `requested`, `acknowledged` by the server's reply, `confirmed` by a read-back from the system of record, or `contradicted`; a profile declares `acknowledge` and `confirm`, the gateway wires read-backs, and `effect_confidence` and `verdict_basis` let VERIFY say what each verdict rests on. A server reporting a different effect than the one requested is now visible (DEC-057, DEC-058, LIM-065, LIM-066).
- **Report P1b-4.** With a language-model attacker and reasoning targets, zero protected violations over 1,670 evaluable episodes in seven arms of four local models, and no detectable effect of reasoning in three pairs. Two reasoning-on arms gate-failed; qwen3.5-9b was re-run with reasoning requested explicitly (`--target-enable-thinking`), and smollm3-3b cannot reason with a system prompt. Both deviations are logged (FIND-058, FIND-059, DEC-059, LIM-067).
- Add `docs/phantom-b.md`, a PHANTOM-B pass over a VAIS deployment, with the paper's expense example as a test.
- **Fix the continuous campaign's declared rules.** They did not know policy v6 or the two new invariant types, so the release campaign classified a correct load as a real fault. The sets are current, and a test now fails whenever a loader accepts a version or type the campaign does not declare (FIND-060).
- Regenerate both benchmark snapshots. Beyond the version, and the adaptive configuration hash that covers it, nothing changed; recomputing that hash with the version set back to rc12 reproduces rc12's value.
- Add `docs/rc12-to-rc13-diff.md` and `benchmarks/rc/v0.12.0rc13-release-review.json`.

## 0.12.0-rc12 - 2026-09-22

A review of the monitor's edges rather than its decision function, prompted by a second external reviewer: what is passed into the monitor, what the verifier is told, what persists and what is claimed. Nothing bypassed a DENY. Seven defects were found and fixed, each with a regression test that fails when its fix is reverted (23 mutants, all killed), and eleven limitations were published, including every point the reviewer raised.

- **Fix consume-once on the MCP path.** `MCPProtectedClient` took no `ApprovalStore` and called the monitor without one, so the monitor fell back to the contract's approved fingerprints, which never consume, and one approval authorized the same MCP call every time it was proposed. The client now accepts `approval_store` and forwards it (FIND-049). Without a store, a contract-held approval is still reusable for the life of the contract on either path (LIM-044).
- **Fix ENFORCE and VERIFY disagreeing about approvals.** The monitor consumes store grants, but `exact_action_approval` looked only at contract-held approvals, so a payment correctly approved through the store was allowed and then reported as `effect_not_exactly_approved`. `DeclarativeInvariantEngine.evaluate` and `DeclarativeVerifiableReward.evaluate` accept the store; a consumed grant for the same identity counts as approval and an unconsumed one does not (FIND-050, DEC-041).
- **Add `approval_single_use`**, an aggregate invariant reporting each exact approval that authorized more than one effect above its threshold, and add it to the default set. `exact_action_approval` could not see how many times one approval was used, so on the contract path one approval replayed across two identical payments passed twice while the monitor allowed both. A genuine re-approval of an identical action is also reported, because the store cannot show that it was approved twice (FIND-051, DEC-042).
- **Fix `FrozenDict` accepting `|=`.** It blocked `update` and the other mutators, but `dict.__ior__` is implemented in C and bypassed them, so an in-place union rewrote a contract's bound recipient or an action's fingerprinted arguments. A model cannot reach this; integration code can, by accident (FIND-052).
- **Fix three loader contract faults.** The continuous campaign found `trust_required` tested with set membership, so an unhashable value raised `TypeError` (FIND-053, the FIND-047 pattern one field over). Reading the MCP loader then found a `:` in a server or tool name escaping as `ValueError` (FIND-054), and NFC-equal effect field names merged silently with the later one winning, so the file and the loaded mapping could disagree (FIND-055). The MCP loader is not a campaign target (LIM-054).
- **Stop scoring indeterminate calls as defended.** A call that fails after dispatch produces no effect, so the verifier saw nothing and the episode scored zero. Absent a verified violation, all three scorers now remove such an episode from the evaluable set and count it separately, as they already did for target failures. Summaries gain `indeterminate_episodes` and `mcp_indeterminate_episodes`; `mcp_indeterminate_action_count` is now counted over every episode, since an indeterminate episode is no longer in the evaluable set. Every published run recorded zero, so no published result changes (DEC-040).
- **Record which action was decided, and for whom.** Audit events on `ProtectedExecutor` and `MCPProtectedClient` now carry the action fingerprint and the contract's principal, session, tenant and capability, never argument values. `MCPProtectedClient` accepts an optional `audit` trail, including denials made before the monitor is consulted and indeterminate outcomes recorded by exception class only (DEC-043).
- Publish as limitations: consume-once holds within one store instance, not across processes (LIM-045); an MCP effect is the dispatched request rather than resulting state (LIM-046); authority has no freshness, revocation or expiry (LIM-047); authority is judged per argument and actions carry no provenance, so the default policy accepts an untrusted payment amount up to its approval threshold (LIM-048); a malicious MCP server is out of scope (LIM-049); complete mediation is assumed rather than tested (LIM-050); model text and reasoning output are unmediated (LIM-051); `forbidden_values` is a canary tripwire rather than a control (LIM-052); and re-binding model output to trusted contract values is an integration step, not a library function (LIM-053).
- Correct the documents that claimed more than the code: consume-once in `docs/naming.md` and the README, the verifier's independence from the executor in `docs/naming.md` and `docs/architecture.md`, and the complete-mediation assumption in `docs/threat-model.md`, which now names both enforcement paths, lists the known gaps and adds unmediated text and malicious servers as non-goals. Add S15 and S16 to `docs/ATTACK-SURFACE.md`.
- Regenerate both benchmark snapshots. Beyond the version, and the adaptive configuration hash that covers it, the only change is the new indeterminate counters, zero everywhere.
- Add `docs/rc11-to-rc12-diff.md` and `benchmarks/rc/v0.12.0rc12-release-review.json`.
...
- Test the provenance lattice against its stated rule rather than its implementation (P1-3). Expected labels are computed by a function written from the `taint.py` docstring, then applied to seeded DAGs of depth at least twelve with mixed trusted and untrusted roots. No laundering path was found through derivation. Five separate mutations of the lattice rules were each caught by the suite, so the tests are known to be capable of failing (FIND-044, DEC-036).
- Record the asymmetry those tests surfaced. Integrity defaults to the cautious end of the lattice, since MCP data is untrusted unless something trusted vouches for it and a derivation with no inputs is `derived_untrusted`. Confidentiality defaults to `public`, which is the bottom of the lattice. A `confidentiality_ceiling` invariant fails closed when effect provenance is absent and cannot fail closed when provenance is present and understates the true level, so confidentiality enforcement is only as good as the application's labelling at the point data enters the system (LIM-036).
- Run fork, truncate, splice and reorder against the audit chain (P1-4). The detection boundary is not drawn by which manipulation is used but by whether the attacker recomputes the chain afterwards, which needs only public SHA-256 because the chain holds no key. In-place edits to any of eight fields, dropping events from the front, and naive reorder or splice are caught, and each of the verifier's three checks is tripped in isolation so that removing any one of them fails the suite (FIND-045).
- Record what the chain does not detect. Truncating the tail passes verification and needs no rebuild at all. Splicing a forged `allow`, deleting an event, or reversing the chain all pass once rebuilt. A fork is invisible from either branch by construction. Each is asserted by a test that is expected to fail if a signed head or external commitment is ever added (LIM-037, DEC-037).
- Record that `AuditTrail.verify` returns a bare boolean with no locus, so an investigator cannot distinguish one edited record from a wholly rewritten prefix (LIM-038).
- **Fix a fail-open in the approval store found by fault injection (P1-5).** `ApprovalStore.consume` marked a grant spent in memory and then persisted it. When the write failed, memory said spent while the stored record said unspent; the call itself failed closed, but the next process to load the store read the grant as unspent and consumed it again, so consume-once did not survive a failed write across a restart. The in-memory change and the persist are now applied together, with the in-memory change rolled back if the write fails (FIND-046, DEC-038).
- Add the fault-injection matrix. Config faults all fail closed, and an empty policy file loads as deny-all rather than allow-all. An invariant referencing an absent effect field reports `missing_effect_provenance`, and an evaluator that raises propagates rather than reading as no violations.
- Record the three cells with no mechanism to test: there is no clock in the enforcement path, so clock skew cannot move a decision; there is no timeout, so a fault that never returns hangs instead of denying (LIM-040); and out-of-memory is not reproducible in a unit test.
- Record that a policy file which is not valid YAML surfaces as `yaml.YAMLError` rather than `PolicyValidationError`, so an application catching the latter to degrade gracefully will not catch it (LIM-039), and that an invariant naming an effect kind the application never emits is silently inert (LIM-041).
