...
0.12.0rc13 also gives actions an origin (P1b-6): the join of the labels visible when the action was planned, set by both LLM adapters and by the gateway, carried onto effects, and excluded from the fingerprint. Policy v6's `untrusted_origin` lets a tool require a trusted origin, escalating to an approval or denying otherwise, and the `trusted_origin` invariant checks the same in VERIFY. It is opt-in, and measurement on RC7 found it gates a tool rather than detecting an attack (FIND-057).
